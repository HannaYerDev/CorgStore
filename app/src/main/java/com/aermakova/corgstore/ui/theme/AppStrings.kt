package com.aermakova.corgstore.ui.theme

object AppStrings {
    // Boarding screen
    const val boardingTitle1: String = "Make Your"
    const val boardingTitle2: String = "home beautiful"
    const val boardingDescription: String =
        "The best simple place where you discover the most wonderful furniture and make your home beautiful"
    const val getStarted: String = "Get Started"

    const val homeTitle1: String = "Make home"
    const val homeTitle2: String = "beautiful"
    const val filterPopular: String = "Popular"
    const val filterChair: String = "Chair"
    const val filterTable: String = "Table"
    const val filterArmchair: String = "Armchair"
    const val filterBed: String = "Bed"
    const val filterLamp: String = "Lamp"
    const val filterStorage: String = "Storage"
}